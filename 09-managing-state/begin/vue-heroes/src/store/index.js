...
import { ADD_HERO, DELETE_HERO, GET_HEROES, UPDATE_HERO } from './mutation-types';
...
const actions = {
    ...
    async addHeroAction({ commit }, hero) {
        const addHero = await dataService.addHero(hero);
        commit(ADD_HERO, addHero);
    },
    async deleteHeroAction({ commit }, hero) {
        const deleteHeroId = await dataService.deleteHero(hero);
        commit(DELETE_HERO, deleteHeroId);
    },
    async updateHeroAction({ commit }, hero) {
        const updateHero = await dataService.updateHero(hero);
        commit(UPDATE_HERO, updateHero);
    },
};
const getters = {
